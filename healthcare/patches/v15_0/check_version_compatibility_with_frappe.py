...
def execute():
	...
	# 	message = f"""
	# 		The `develop` branch of xx Health is no longer compatible with Frappe & ERPNext's `version-14`.
	# 		Since you are using ERPNext/Frappe `version-14` please switch xx Health app's branch to `version-14` and then proceed with the update.\n\t
