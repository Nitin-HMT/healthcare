      ...
      <div class="grid grid-cols-2">
        <div class="m-1 bg-white rounded-xl drop-shadow-lg p-2">
          ...
          <div v-if="appointment.list && appointment.list.data.length">
            <div class="grid grid-cols-4 border text-base bg-gradient-to-br from-teal-700/85 to-teal-900 text-white py-1 px-1 rounded-md">
                <div>Name</div>
                <div>Doctor</div>
                 <div>Status</div>
                 <div class="flex justify-end items-end">Payment</div>
            </div>
            <div v-for="item in appointment.list.data" class="text-xs grid grid-cols-4 justify-start items-center py-1 px-1.5">
              <div>{{item.name}}</div>
              <div>{{item.practitioner}}</div>
              <div>
                <Badge variant="subtle" :theme="statusToColor[item.status]">{{ item.status }}</Badge>
              </div>
              <div v-show="!item.fee_valid" class="flex justify-end items-end">{{formatCurrency((item.paid_amount) ,"INR")}}</div>
              <div v-show="item.fee_valid" class="flex justify-end items-end">Free Follow-Up</div>
            </div>
            
          <div class="flex items-end justify-end">
            <Button @click="appointment.next()" v-show="appointment.hasNextPage" :variant="'outline'" theme="green">More Records</Button>
            </div>
            <div class="flex items-end justify-end">
            <Button @click="appointment.reload()" v-show="!appointment.hasNextPage" :variant="'outline'" theme="green">Refresh</Button>
            </div>
          </div>
          <div v-else class="font-mono text-xs">No Appointment</div>
        </div>
        <div class="m-1 bg-white rounded-xl drop-shadow-lg p-2">
          <div>Labs History</div>
          <LabHistory :Pat_id="pat_id"  :Request_from= "route.name"/>
        </div>        
        <div class="m-1 bg-white rounded-xl drop-shadow-lg p-2 row-span-2">
...
import {Badge, FormControl,Button,createListResource, createDocumentResource,Dialog,Textarea} from 'frappe-ui';
