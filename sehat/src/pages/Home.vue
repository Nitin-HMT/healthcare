<template>
    ...
    <nav class="mt-4 grid md:grid-cols-3 gap-2 p-1 mx-3">
        ...
        <div :required="true" v-if="data.clinic_type=='Roving'">
        <FormControl 
          required
          :type="'text'"
          :ref_for="true"
          size="sm"
          variant="subtle"
          placeholder="Alternate Clinic Name"
          :disabled="false"
          :modelValue="data.roving_hosp"
        />
          
      </div>
      <div v-if="data.clinic_type!='Roving'">
        <Autocomplete 
            :options="appoints" 
            v-model="data.appnt_patient" 
            placeholder="Appointment Queue" 
            :multiple="false"
            >
        ...
        <div>
          ...
          <span>
            <TabButtons
            :buttons="[
                      { label: 'Patient Advise', value: 'Advise' },
                    ]"
             v-model="data.work_area"
          />
          </span>
          -->
        ...
        <Button
          ...
          <Button
            ...
            :link="null"
          >
          <FeatherIcon class="w-6 h-6 rounded-full text-gray-700"  name="trash-2"/>
          </Button>
        </div>
    </nav>
    <!--{{ data.appnt_patient }}
     selected patient information display -->
    <div v-if="data.appnt_patient" class="p-3 grid md:grid-cols-5 gap-2 shadow-md bg-gray-100/80 rounded-sm space-y-2 justify-between flex-col hover:bg-violet-100/20 transition-all">
      <span class="flex gap-2 col-span-2" @click="patDialogshown=true">
        <FeatherIcon class="w-8 h-8" :class="[
            Patient_details.list.data[0].gen_abbr === 'F' ? 'text-pink-700' :'text-blue-700'
        ]"  name="user"/>
       <p class="font-mono font-bold underline decoration-gray-600 flex gap-4">Patient Details:</p>
        <p class="flex items-center justify-between flex-col font-sans" v-if="Patient_details">{{ data.appnt_patient.label }}| {{  Patient_details.list.data[0].gen_abbr }} | {{ data.appnt_patient.description }}</p>
    ...
    <div class="p-3 grid md:grid-cols-5 gap-2 font-sans shadow-md bg-gradient-to-r from-blue-800/30 to-blue-900/40 rounded-sm space-y-2 justify-between flex-col" v-if="Patient_details">
    <span class="flex items-center col-span-5  text-gray-900" >
        <Tabs as="div"  
        :tabs="[
          {
            label: 'History',
    ...
    <div>
    <div  v-if="Patient_details" @click="moreinfoDialog=true" class="py-2 m-1 flex items-center justify-between flex-col col-span-2 mt-4 px-3 shadow-md bg-teal-100/50 rounded-sm hover:bg-teal-200 transition-all">
  ...
  <!--Search Bar-->
    <div v-if="Patient_details">
      <div class="px-[30px] py-8 mx-6 grid grid-cols-12 gap-2">
        <div class="col-span-10">
          ...
          >
            ...
            <!--<FeatherIcon
              ...
              name="search"
            />-->
            <FeatherIcon class="w-6 h-6" :class="[data.appnt_patient.gender === 'Female' ? 'text-pink-700' :'text-blue-700']"  name="search"/>
            </template>
          </Autocomplete>
        </div>
        <div class="col-span-2 gap-2 flex">
          <div><Button
            ...
            :link="null"
          >
          <FeatherIcon class="w-6 h-6" :class="[data.appnt_patient.gender === 'Female' ? 'text-pink-700' :'text-blue-700']"  name="plus"/>
          </Button></div><div>
      ...
      ><FeatherIcon class="w-6 h-6" :class="[data.appnt_patient.gender === 'Female' ? 'text-pink-700' :'text-blue-700']" name="send"/></Button>
    </div></div>
          ...
          <ul class= "space-y-2">
              <li v-for="(item,index) in data.selected" > 
                <span v-if="item.type=='surg'" class="grid md:grid-cols-3 gap-2">
                  {{item.label}} 
                <FormControl v-model="item.additional_attr" type="date" placeholder="Date" />
  ...
  <Dialog :options="{
      ...
      v-model="patDialogshown">
      <template #body-content>
                <span class="grid md:grid-cols-2 gap-2 p-1 font-sans">
                  <div >{{Patient_details.list.data[0].patient_name}}, {{Patient_details.list.data[0].sex}}, {{ data.appnt_patient.description }}</div>
                  <div class="flex items-center justify-between">
                  {{Patient_details.list.data[0].mobile}}</div><!--<FormControl v-model="Patient_details.list.data[0].aged" type="text" label="Age (Years)" placeholder="Age" />
                  -->
                  
                  <div class="col-span-2 flex items-center justify-between gap-2">
                  <FormControl v-model="data.appnt_patient.Vitals[0]" type="text" label="BP" placeholder="BP" />
                  <FormControl v-model="data.appnt_patient.Vitals[1]" type="text" label="Pulse" placeholder="HeartRate" />
                  <FormControl v-model="data.appnt_patient.Vitals[2]" type="text" label="SpO2 (%)" placeholder="SpO2" />
                </div><div class="col-span-2 flex items-center justify-between gap-2">
                  <FormControl v-model="data.appnt_patient.Vitals[3]" type="text" label="Ht (m)" placeholder="Height" />
                  <FormControl v-model="data.appnt_patient.Vitals[4]" type="text" label="Wt (Kg)" placeholder="Weight" />
                  <FormControl v-model="data.appnt_patient.Vitals[6]" type="text" label="Temp" placeholder="Temperature" />
                  <p>{{data.appnt_patient.Vitals[9]}}</p>
                 
                </div>
                  <!--<p >{{Patient_details.list.data[0]}} </p>
                   <FormControl type="autocomplete" :options="referdr" v-model="data.appnt_patient.Vitals[9]" label="Referred By" placeholder="Referred By"/>
                  BMI: {{ data.appnt_patient.Vitals[5] }}-{{ data.appnt_patient.Vitals[8] }}
                   <FormControl v-model="Patient_details.list.data[0].mobile" type="text" label="Mobile" placeholder="Mobile"/>
                  <p >{{data.appnt_patient}} </p>
                <FormControl v-model="item.additional_attr" type="text" placeholder="Date" />
                <Select :options="med_dosage" v-model="item.additional_attr" placeholder="Dosage" />
                <Select :options="med_duration" v-model="item.additional_attr_2" placeholder="Duration" />
                <p>{{ item.sp_attr }}</p>-->
                ...
                <!--<div class="grid md:grid-cols-2 gap-2 p-1">
                  <p class="col-span-2 font-sans font-semibold">Patient History</p>
                  <p class="col-span-2 font-sans text-sm font-light">Please Follow this Format Only : Description, since, comments;</p>
                  <FormControl v-model="Patient_details.list.data[0].medical_history" type="textarea" label="Pre-Existing" placeholder="Description, since, comments; Description..." />
                  <FormControl v-model="Patient_details.list.data[0].surgical_history" type="textarea" label="Surgeries" placeholder="Description, since, comments; Description..." />
                  <FormControl v-model="Patient_details.list.data[0].allergies" type="textarea" label="Allergies" placeholder="Description, since, comments; Description... " />
                  <FormControl v-model="Patient_details.list.data[0].medication" type="textarea" label="Medication" placeholder="Description, since, comments; Description..." />
  ...
  <Dialog v-model="submit_confirm">
    ...
    <template #body-content>
      <p>Are You Sure you want to submit this Patient Advise and Move to the next patient?</p>
    </template>
    <template #actions>
  ...
  <FormControl v-model="add_item.labs" type="text" placeholder="Lab Name" />
...
<script setup>
    ...
    const appointment = createListResource({
    ...
    Patient_details = createListResource({
        ...
        fields: ["*"],
        filters: {
            name: data.appnt_patient.patient_id
        },
        pageLength: 1,
        auto: true,
    }); 
    ...
    const make_opd_prescription = createResource({
        ...
        makeParams(){
            return {
                ...
                appointment: data.appnt_patient,
        ...
        updateSearches();
        createToast({
            title: 'Patient Advise Created'+data.appointment_name,
            icon: 'check',
