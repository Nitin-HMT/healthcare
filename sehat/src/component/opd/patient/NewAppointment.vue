<template>
  ...
  <Button :variant="'solid'" class="justify-self-end bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white transition-all"
          size="sm" :disabled="false" v-if="appoint_flag" @click="appoint_flag=false"><X/></Button></div>
  <div class="p-1 mt-1 bg-gradient-to-br from-teal-50 via-white to-teal-100 rounded-md">
    <div class="mt-1 flex justify-center items-center font-semibold tracking-wider text-lg">New Appointment</div>
<PatientSearch class="bg-white"/>

<div v-if="!patientStore.sel_pat" class="mt-4 flex item-center justify-center gap-3 p-3">
    <Badge :variant="'solid'" class=" block w-full rounded-full px-2 py-2 font-serif item-center justify-center
    bg-gradient-to-r from-teal-800 to-teal-700 text-white transition-all" size="lg" label="No Patient Selected"/>
</div>
<div v-else class="grid grid-cols-2 gap-5 m-4 text-sm">
    <div class="flex flex-col gap-5">
      <Autocomplete :options="healthcare_pr" v-model="data.doctor" class="block w-full rounded-full px-2 py-2 font-serif 
      bg-gray-100 border-solid border border-gray-100 hover:bg-gray-200/90 transition-all" placeholder="Appointment For">
      </Autocomplete>
      <span class="block w-full rounded-full px-2 py-2 font-mono 
      ...
      <Switch class="block w-full rounded-full py-2 font-serif 
    ...
    <div class="flex flex-col gap-5">
      ...
      {label: 'Procedure/OT', disabled: true,},]" v-model="data.appnt_type" class="block w-full rounded-full px-2 py-2 font-serif 
      bg-gray-100 border-solid border border-gray-100 hover:bg-gray-200/90 transition-all"/>
      ...
      <FormControl type="autocomplete" :options="referdr" v-model="selrefdr" placeholder="Referred By" class="block col-span-3 w-full rounded-full px-2 py-2 font-serif 
      bg-gray-100 border-solid border border-gray-100 hover:bg-gray-200/90 transition-all"/>
      <Button :variant="'subtle'" size="xl" label="+" class="w-full rounded-full py-2 mt-1 
      bg-gray-300 border-solid border hover:bg-gray-400 text-lg transition-all" @click="refer_create=true"/></div>
    ...
    <Button v-if="data.doctor" :variant="'subtle'" size="xl" label="Book Appointment" class="col-span-2 block w-full rounded-full px-2 py-2 font-serif item-center justify-center 
      bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 text-white transition-all" @click="make_newAppoint(data,patientStore.sel_pat)" :loading="appoint_load"/>
    <Button v-else :variant="'subtle'" size="lg" label="Doctor Not Selected" 
      class="col-span-2 block w-full rounded-full px-2 py-2 item-center justify-center" :disabled="true" /></div>
</div>
...
<FormControl v-model="data.refer[1]" type="textarea" label="More Info" placeholder="More Info eg Speciallity, Mobile No." />
  ...
  <template #actions>
    <Button variant="solid" @click="make_newRef(data.refer[0],data.refer[1])" v-if="data.refer[0]">
      Confirm
    </Button>
  </template>
</Dialog>
...
import { createListResource,Autocomplete,Switch,createResource,FormControl, Badge,Dialog,ErrorMessage } from 'frappe-ui';
...
const { healthcare_pr, make_newAppoint, error_appoint,appoint_create,appoint_load,appoint_flag }= appoint();
