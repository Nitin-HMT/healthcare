    ...
    <div>
      <div class="grid grid-cols-12 gap-2 p-2">
        ...
        <div
          ...
          class="col-span-8 block w-full rounded-full px-2 py-2 bg-gray-100 border-solid border border-gray-100 hover:bg-gray-200/90 transition-all"
        >
          <Autocomplete
        ...
        <div>
          ...
          >
            <UserPen
              class="w-6 h-6 rounded-full text-gray-700"
              v-if="patientStore.sel_pat"
            />
            <UserPlus
              class="w-6 h-6 rounded-full text-gray-700"
  ...
  >
    <template #body-content>
      <div class="text-md font-sans space-y-1">
        <span>
          <p
            class="font-mono text-gray-600 font-light flex items-center justify-end"
          >
            {{ patientStore.sel_pat.name }}
          </p>
          <div
            class="grid md:grid-cols-3 gap-4 border-b border-grey-50/80 pb-3"
          >
            <FormControl
              v-model="data.new_patient[0]"
              :required="true"
              label="First Name"
              type="text"
            ...
            />
            <FormControl
              v-model="data.new_patient[1]"
              label="Middle Name"
              type="text"
              placeholder="Middle Name"
            />
            <FormControl
              v-model="data.new_patient[2]"
              label="Last Name"
              type="text"
          ...
          <div class="grid md:grid-cols-2 gap-4 mt-3">
            ...
            <FormControl
              ...
              :options="data.blood"
              label="Blood Group"
              v-model="data.new_patient[5]"
              placeholder="Blood Group"
            />
            <FormControl
              v-model="data.new_patient[6]"
              :required="true"
              label="Mobile"
              type="text"
              placeholder="Mobile"
    ...
    <template #actions>
      <div class="flex flex-row justify-start items-center">
        <div v-if="data.new_patient[6] && data.new_patient[6].length == 10">
          <Button
            @click="make_patient.submit()"
            v-if="data.new_patient[6].length == 10"
            :loading="make_patient.loading"
            variant="solid"
            class="bg-gradient-to-r from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white transition-all"
          >
            Save/Update
          </Button>
        </div>
        <div v-else class="text-xs text-red-800">
          Mobile Number Incorrect (10 Digits only)
        </div>
        <Button variant="subtle" class="ml-2" @click="clear">Clear</Button>
        <Button
          class="ml-2 bg-gradient-to-r from-red-800 to-red-600 hover:from-red-900 hover:to-red-700 text-white transition-all"
          @click="add_patient = false"
          >Close</Button
        >
      </div>
    </template>
...
const make_patient = createResource({
  ...
  onSuccess: (d) => {
    ...
    if (patientStore.sel_pat) {
      ...
      if (patientStore.sel_pat) {
        console.log("oid" + patientStore.sel_pat.i);
      }
