          ...
          <div>
            ...
            <span v-else>
              ...
              <span v-else class="grid grid-cols-2 gap-4">
                <p>Consult Charges: {{formatCurrency(appointStore.sel_app.bill ,"INR")}}</p>
                <p>Total Payable : {{formatCurrency((appointStore.sel_app.bill-discount) ,"INR")}}</p>
                <Switch size="sm" label="Free Visit" v-model="appoint.fee_valid"/>
                ...
                <Button v-if="appoint.mode_of_payment && (appointStore.sel_app.bill-discount)>0" :variant="'solid'" @click="make_paid(); pay_load=true;" :loading="pay_load" class="bg-gradient-to-r from-green-600 to-green-900">Make Invoice & Payment</Button>
