<template>
          ...
          <div class="p-1 bg-gradient-to-br from-gray-50 via-white to-gray-100 rounded-md">
        <div class=" p-1 grid grid-cols-3 gap-2">
          <div class="flex-row col-span-2"><b>{{vitals_all.patient}}</b> | {{ vitals_all.gender }} | {{ vitals_all.age }} yrs</div> 
          ...
          <div class="col-span-3 grid grid-cols-3 gap-2" v-if="vitals_all.pos">
            ...
            <Button :variant="'solid'" @click="make_free(); pay_load=true;" :loading="pay_load">Update</Button>     
          </div>
          <div v-else class="col-span-3">
            <span v-if="vitals_all.invoice" class="flex flex-col justify-between items-center">
              <div class="text-base text-red-900 my-2">To change Payments, First the previous Payment Should be cancelled and then a new Payment Created.</div> 
                <Button :variant="'solid'" @click="cancel(vitals_all.inv_id); pay_load=true;" :loading="pay_load" class="bg-gradient-to-r from-red-600 to-red-900">Cancel Previous Payment</Button>
            </span>
            <span v-else class="grid grid-cols-2 gap-4">
              <p>Consult Charges: {{formatCurrency(vitals_all.bill ,"INR")}}</p>
              <p>Total Payable : {{formatCurrency((vitals_all.bill-discount) ,"INR")}}</p>
              <Switch size="sm" label="Free Visit" v-model="vitals_all.pos"/>
              <FormControl :type="'number'" size="sm"  variant="subtle" label="Discount Amount" v-model="discount"/>

              <FormControl :type="'select'" :required="true" :options="[{label: 'Cash',value: 'Cash',},
              {label: 'UPI',value: 'UPI',},{label: 'Credit Card',value: 'Credit Card',},
              {label: '3rd Party',value: '3rd Party',},]"
               size="sm"  variant="subtle" label="Mode of Payment" v-model="vitals_all.pay_mode"/>
              <div/>
              
              <Button v-if="vitals_all.pay_mode && (vitals_all.bill-discount)>0" :variant="'solid'" 
                @click="make_paid(); pay_load=true;" :loading="pay_load" 
                class="bg-gradient-to-r from-green-600 to-green-900">Make Invoice & Payment</Button>
...
import { ref } from 'vue';
import { Switch,Button,FormControl,createListResource,createResource } from 'frappe-ui';
