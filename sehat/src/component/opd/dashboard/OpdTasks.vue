...
<div class="grid grid-cols-2 gap-1" v-if="!vital_flag && !pay_flag">
    ...
    <ol class="rounded-xl text-sm font-medium border gap-2 py-2 my-1 bg-gradient-to-r from-teal-700/80 to-teal-700/90 text-white">
        <div class="pl-3 flex gap-2 col-span-2">
        <div>Vitals</div>
        </div>
    </ol>
    <div class="pr-1.5">
    <ol v-for="(item,index) in appointStore.vitals_pending"  @click="vitalsqueue(item)"
    ...
    <div class="" v-if="appointStore.payment_pending.length>0" v-show="false">
    <ol class="rounded-xl grid grid-cols-2 text-sm font-medium border gap-2 py-2 my-1 bg-gradient-to-r from-teal-700/90 to-teal-700/80 text-white">
        <div class="pl-3 flex gap-2 col-span-2">
        <div>Pending Payment</div>
        </div>
    </ol>
    <div class="pl-1.5">
    <ol v-for="(item,index) in appointStore.payment_pending"  @click="payqueue(item)"
    class="grid grid-cols-2 text-sm border gap-2 py-2 my-1 hover:shadow-md hover:bg-gradient-to-r from-teal-200/20 to-teal-300/80 rounded-full">
        ...
        <div class="justify-self-end pr-2" v-if="!item.pos && !item.invoice">₹ {{ item.bill }}</div>
    </ol>
    </div>
    </div>
    <div class="col-span-2" v-if="appointStore.payment_complete.length>0" v-show="false">
    <ol class="rounded-xl grid grid-cols-2 text-sm font-medium border gap-2 py-2 my-1 bg-gradient-to-r from-gray-700/70 from-0% to-teal-700/90 to-90% text-white">
        <div class="pl-3 grid grid-cols-2 col-span-2" @click="exp_pay_hist=true" v-if="!exp_pay_hist">
        <div>Today's Payment History</div>
        <ChevronDown class="justify-self-end pr-2 h-4 w-8"/>
        </div>
        <div class="pl-3 grid grid-cols-2 col-span-2" @click="exp_pay_hist=false" v-if="exp_pay_hist">
        <div>Today's Payment History</div>
        <ChevronUp class="justify-self-end pr-2 h-4 w-8"/>
        </div>
    </ol>
    <div class="pl-1.5 grid grid-cols-2 gap-2" v-if="exp_pay_hist">
    <ol v-for="(item,index) in appointStore.payment_complete"  @click="payqueue(item)"
    class="grid grid-cols-2 text-sm border gap-2 py-2 my-1 hover:shadow-md hover:bg-gradient-to-r from-teal-200/20 to-teal-300/80 rounded-full">
        <div class="pl-3 flex gap-2 col-span-1">
        ...
        <div class="justify-self-end pr-2 text-xs text-amber-500" v-if="item.pos"><i>No Charge</i></div>
        <div class="justify-self-end pr-2 text-xs text-green-500" v-if="item.invoice">₹ {{item.bill}}</div>
    </ol>
    </div>
    </div>
    <div/>
...
<div v-if="vital_flag" class="grid grid-cols-1">
    <Button @click="vital_flag=false" class="justify-self-end flex bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white transition-all"><X/></Button>
    <Vitals/>
    
</div>
<div v-if="pay_flag" class="grid grid-cols-1">
    <Button @click="pay_flag=false" class="justify-self-end flex bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white transition-all"><X/></Button>
 <Payments/>
