...
* Procedure, 
! Allergy
"" Descriptive Comments' class="h-64 rounded-lg text-sm" 
        ...
        <div class="space-y-2">
            ...
            <FormControl type="autocomplete" :options="referdr" v-model="selrefdr" placeholder="Refer To" 
                class="col-span-7 rounded-full"/>
            <Button :variant="'subtle'" size="md" label="+"
                class="rounded-full bg-white border border-teal-800 text-teal-800" @click="refer_create=true"/>
            </div>
            <FormControl :type="'number'" :ref_for="true" size="sm" placeholder="Follow-up (In Days)"
                v-model="interaction.follow_up"/>
        </div>
        </div>
        <div class="col-span-1">
        <div class="col-span-1 space-y-1.5 tracking-wide capitalize" v-if="result_grouped">
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.Symptoms">
              <p  class="font-semibold underline decoration-red-800 decoration-2">Symptoms:</p>
              <div v-for="item in result_grouped.Symptoms">
                {{item.Display}}<Badge :variant="'outline'" size="sm" theme="green" @click="create_new_element(item)" class="cursor-pointer" v-if="item.new">New</Badge>,
            </div>
          </div>
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.Diagnosis">
              <p  class="font-semibold underline decoration-red-600 decoration-2">Diagnosis:</p>
              <div v-for="item in result_grouped.Diagnosis">
                {{item.Display}}<Badge :variant="'outline'" size="sm" theme="green" @click="create_new_element(item)" class="cursor-pointer" v-if="item.new">New</Badge>,
              </div>
            </div>
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.labs">
              <p class="font-semibold underline decoration-amber-600 decoration-2">Labs:</p>
              <div v-for="item in result_grouped.labs">
                {{item.Display}}<Badge :variant="'outline'" size="sm" theme="green" @click="create_new_element(item)" class="cursor-pointer" v-if="item.new">New</Badge>,
              </div>
            </div>
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.surg">
              <p class="font-semibold underline decoration-cyan-800 decoration-2">Procedure:</p>
              <div v-for="item in result_grouped.surg">
                {{item.Display}}<Badge :variant="'outline'" size="sm" theme="green" @click="create_new_element(item)" class="cursor-pointer" v-if="item.new">New</Badge>,
              </div>
            </div>
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.Meds">
              <p  class="font-semibold underline decoration-green-700 decoration-2">Medicine:</p>
              <div v-for="item in result_grouped.Meds">
                {{item.Display}}<Badge :variant="'outline'" size="sm" theme="green" @click="create_new_element(item)" class="cursor-pointer" v-if="item.new">New</Badge>,
              </div>
            </div>
            <div class="flex flex-wrap gap-1 py-0.5" v-if="result_grouped.Unknown"><p class="font-semibold">Uncategorised:</p>
            ...
            <div class="flex flex-wrap gap-1 py-0.5" v-if="comments"><p class="font-semibold">Comments:</p>
              <div>
                  {{comments}}
              </div>
            </div>  
            <div class="grid grid-cols-1 gap-1 py-2" v-if="result_grouped.Allergy">
        ...
        <div class="m-3 flex flex-grow items-end justify-end pr-8" v-if="result_grouped||interaction.follow_up||selrefdr.label">
            <Button :variant="'subtle'" size="md" label="Review"
                class="rounded-full bg-white border border-teal-800 text-teal-800" @click="interation_direct(interaction.notes); review=true; patientPanel_flag=true;vitals_flag=false"/>
        </div> 
        </div>
        
    </div>
    <div class="flex flex-row capitalize border-2 border-dotted rounded border-gray-500 p-2" v-if="review">
      <div>
        ...
        <div class="col-span-1 grid grid-cols-4 gap-2 gap-x-3 tracking-wide" v-if="result_grouped">

          <div class="col-span-4 grid grid-cols-8 gap-1 border border-red-600 rounded-lg pb-1" v-if="result_grouped.Unknown">
              <p class="text-sm font-medium px-1 gap-2 pl-1 my-1 col-span-8 underline">Uncategorised:</p>
              <div class="text-sm font-medium tracking-wide text-red-600 pl-2 pb-1 col-span-8 ">
                Couldn't Transcribe these, Please Review them, Create a new Medical Terminology if Required</div>
              <div v-for="(item,index) in result_grouped.Unknown" class="flex flex-wrap gap-x-2 pt-1 pl-2 align-baseline">
               <Pencil class="h-4 text-teal-800 cursor-pointer -mr-1 " @click="phraseCorrection(item)"/>
                {{item.Functional}}
               <Badge :variant="'outline'" size="sm" theme="orange" @click="create_new_element(item)" class="cursor-pointer">Fix</Badge>
              </div>
            </div>

            <div class="flex flex-col">
              Obx,
              <div class="grid grid-cols-1 gap-1 py-0.5" v-if="result_grouped.Symptoms">
                ...
                <div v-for="(item,index) in result_grouped.Symptoms" class="flex gap-x-2 pt-0.5 pl-2 align-baseline">
                  ...
                  <div v-if="item.new">{{item.Functional}}<Badge :variant="'outline'" size="sm" theme="orange" @click="create_new_element(item)" class="cursor-pointer">Fix</Badge></div>
              ...
              <div class="grid grid-cols-1 gap-1 py-0.5" v-if="result_grouped.Diagnosis">
            ...
            <div class="col-span-3">
              Rx,
            <div class="grid grid-cols-1 gap-1 py-0.5" v-if="result_grouped.labs">
            ...
            <div class="grid grid-cols-1 gap-1 py-0.5" v-if="result_grouped.surg">
            ...
            <div class="grid grid-cols-1 gap-1 py-0.5" v-if="result_grouped.Meds">
               ...
               <Pencil class="h-4 text-teal-800 cursor-pointer -mr-1 " @click="phraseCorrection(item)"/>
                <div v-if="item.new">{{item.Functional}}<Badge :variant="'outline'" size="sm" theme="orange" @click="create_new_element(item)" class="cursor-pointer">Fix</Badge></div>
                  <div v-else class="align-baseline grid grid-cols-6 gap-x-1">
                    <div  class="col-span-2 flex gap-x-2"><div v-if="item.Qualifier[0]" class="pr-0.5">{{ item.Qualifier[0].medicine_form }}</div>
                    {{item.Item[7]}}</div>
                    ...
                    <FormControl :type="'text'" class="col-span-2 flex" disabled="true" v-if="item.Qualifier[0]" size="sm" variant="outline" placeholder="Comments" v-model="item.Qualifier[0].comments"/>
                  </div>
                </div>
            </div>
            <div class="grid grid-cols-1 gap-1 py-2" v-if="result_grouped.Allergy">
        ...
        <div class="grid grid-cols-4 gap-1 py-0.5">
          <div/><div class="col-span-3">
              ...
              <div class="flex flex-col gap-1 pt-1 pl-2 align-baseline">
                ...
                <div v-if="interaction.follow_up"><b>Follow Up</b>: {{ interaction.follow_up }} days</div>
                <div v-if="selrefdr.label"><b>Reffered To:</b> {{ selrefdr.label }}</div>
              </div></div>
        </div>
        <div class="m-3 flex flex-grow items-end justify-end pr-8 gap-x-2">
            ...
            <Button :variant="'subtle'" size="md" label="Back"
                ...
                <Button :variant="'subtle'" size="md" label="Submit" v-if="!stop_submit"
