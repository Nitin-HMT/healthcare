<template>
  <div>
    <div
      v-if="lab_results.data && lab_results.data.length && result.length"
      class="flex-grow border p-1.5 mt-2 pt-2 font-sans text-sm text-gray-900 font-bold leading-5 tracking-wide bg-white rounded-md capitalize drop-shadow-sm"
    >
      ...
      >
        ...
        <div v-for="(items, index) in item.tests">
          <Badge
            :variant="'subtle'"
            class="text-red-900 bg-gradient-to-l from-white to-white border border-red-600"
            size="md"
            >{{ items }}</Badge
          >
        </div>
      ...
      <div
        class="text-sm text-gray-500 flex flex-grow items-center justify-center font-light pt-0.5"
      >
        View Full Results By Clicking History Below
      </div>
    </div>
    <div
      v-else
      class="flex flex-col border mt-2 p-2 justify-center items-center capitalize drop-shadow-sm bg-white rounded-md"
    >
      <div
        class="font-sans text-sm text-gray-600 font-light leading-5 tracking-wide"
      >
        No <u class="px-0.5"> Divergent </u> Labs Reported in the last 7 days.
      </div>
      <div class="text-gray-900 font-sans text-sm" v-if="last_date">
        Last Report Date : {{ last_date }}
      </div>
    </div>
  </div>
</template>
