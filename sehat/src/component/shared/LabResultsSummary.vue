...
<div class="flex flex-row items-center justify-center">
  <Button @click="lab_dialog=true;create_new=false" class="drop-shadow-sm hover:bg-gradient-to-br from-white via-white
   to-purple-200 bg-white hover:bg-white -mt-0.5 mr-1 py-1 px-5 rounded-b-full">
    <span class="hover:text-gray-900 leading-none">Lab History</span>
  </Button>
  <Button @click="lab_dialog=true;create_new=true" class="drop-shadow-sm hover:bg-gradient-to-br from-white via-white to-purple-200 bg-white hover:bg-white -mt-0.5 py-1 px-7 rounded-b-full">
    <span class="hover:text-gray-900 leading-none">+</span>
  </Button>
</div>
    </div>
    
<Dialog v-model="lab_dialog" :options="{size: '5xl'}">
    <template #body-title>
      <h3>Lab Results</h3>
    </template>
    <template #body-content>
      <div class="flex flex-col gap-3">
        <LabHistory :Pat_id="props.Pat_id" :App_id= "props.App_id" :Request_from= "props.Request_from" :create_new="create_new"/>
      </div>
    </template>
    <template #actions>
...
import { FileChartColumnIncreasingIcon } from 'lucide-vue-next';
