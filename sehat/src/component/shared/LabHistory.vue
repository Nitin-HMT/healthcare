<template>
  <div v-if="!props.create_new && !create_result">
    <div class="flex justify-end items-end pb-1"><Button :variant="'outline'" theme="green" @click="create_result=true">+ New Lab</Button></div>
      <div class="text-sm font-light tracking-wide capitalize" v-if="result.length||test_array.length">
        <div class="grid grid-cols-3">
          <Switch size="sm" label="Show Results By Test Results" v-model="show_bytest" v-show="props.Request_from!='pat_dash'"/>
        </div>
          ...
          <template #tab-panel="{ tab }">
            <div class="p-5 grid grid-cols-10">
              ...
              <div class="col-span-9">
                  <div class="text-gray-600 font-thin text-xs flex justify-end items-end pb-1">{{ tab.name }}</div>
                <ul v-for="(item) in tab.tests" class="grid grid-cols-3 border bg-gray-50/60 justify-start items-center my-1 py-1 pl-1 rounded-md">
                  <li class="col-span-1">{{item.test}}</li>
                  <li class="col-span-1">{{ item.result }} {{ item.uom }}</li>
                  <!-- <li>{{ item.uom }}</li> -->
                  <li class="col-span-1">(Ref Range: {{ item.min }}-{{ item.max }})</li>
              ...
              <Button
              @click="lab_tests.runDocMethod.submit({method: 'cancel',name: tab.name})"
              :loadingText="null"
              :link="null"
              :theme="'red'"
              class="bg-white "
            >
            <Trash2 class="w-4 h-4 rounded-full text-red-600"  name="trash-2"/>
            </Button>
        
        </div>
          </template>
        </Tabs>
        <Tabs v-else-if="show_bytest"
          as="div"
          class="border font-seriff"
          :tabs="test_array"
        vertical>
    ...
    <div class="col-span-3 pt-5 ">
                <ul v-for="(item) in tab.tests" class="grid grid-cols-2 justify-start items-center py-1 text-sm">
                  <li>{{item.date}}</li>
      ...
      <div v-else class="font-mono text-xs py-2 flex flex-row items-center justify-between">No Labs</div>
  </div>
<!-- Create a New Lab Test -->
 <div v-else>   
  <div class="font-bold font-serif text-xl pl-2 pr-4 grid grid-cols-5 mt-5">
    <p class="flex col-span-4">Add Lab Results</p>
    <Badge :variant="'solid'" class=" block w-full rounded-full font-serif item-center 
    justify-center bg-gradient-to-r from-gray-800 to-teal-600 hover:from-teal-900 
    hover:to-teal-700 text-white transition-all" size="lg" label="Go Back" @click="create_result=false" v-if="!props.create_new"/>
  </div>
  <div class="grid grid-cols-3 gap-1 font-mono">
    <div class="col-span-2 m-3">
      <div class="grid grid-cols-4 gap-2">
        ...
        <FormControl :type="'autocomplete'"  :options="library.lab_db" v-model="lab_result_line" label="Search Labs" placeholder="Select All Lab Results" :multiple="true"/>
        </div>
      </div>
      <div>
            <ul class= "pt-1">
            <li v-for="(item) in lab_result_line" class="p-0.5"> 
            <span class="grid md:grid-cols-5 gap-1 text-xs font-mono border p-2 hover:drop-shadow-md rounded-md">
              ...
              <FormControl :type="'number'" variant="outline"  placeholder="Min Range" v-model="item.min" />
              <FormControl :type="'number'" variant="outline"  placeholder="Max Range" v-model="item.max" />
              <div></div>
              <div v-if="!item.result" class="font-2xs text-red-700">Add Result</div><div v-else></div>
              <div v-if="!item.uom" class="font-2xs text-red-700">Please Add UOM{{ item.uom }}</div><div v-else></div>
              <div v-if="!item.min" class="font-2xs text-red-700">Add Min Range {{ }}</div><div v-else></div>
              <div v-if="!item.max" class="font-2xs text-red-700">Please Max Range</div><div v-else></div>
          </span>
          </li>
        </ul>
      </div>
  ...
  <div class="pr-2">
        ...
        <span class="grid md:grid-cols-3 gap-2 text-xs font-sans border p-1 hover:shadow-md rounded-md bg-green-100/30">
              ...
              <FormControl :type="'number'" variant="outline"  placeholder="Min" v-model="new_lab[2]" />
              <FormControl :type="'number'" variant="outline"  placeholder="Max" v-model="new_lab[3]" />
...
import { Tabs,Switch,Button,Autocomplete,Badge,Dialog, FormControl,ErrorMessage } from 'frappe-ui';
...
if (existingDateGroup) {
  ...
  result.value.push({
    label: label,
    name: d.name,
    tests: [{ test: test_name, result: d.result, uom: d.uom, min: d.min_normal_range,
