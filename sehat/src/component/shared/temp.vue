<template>
  <div
    v-if="!sel_pat"
    class="mt-4 md:mx-[200px] flex item-center justify-center gap-3 p-3"
  >
    <Badge
      :variant="'solid'"
  ...
  <div v-if="sel_pat">
    <div
      v-if="!result.length && !create_result"
      class="mt-4 md:mx-[200px] flex flex-row item-center justify-center gap-3 p-3"
    >
      <Badge
        :variant="'solid'"
        class="block w-full rounded-full px-2 py-2 font-serif item-center justify-center bg-gradient-to-r from-red-800 to-red-600 hover:from-red-900 hover:to-red-700 text-white transition-all"
      ...
      <Badge
        :variant="'solid'"
        class="block w-full rounded-full px-2 py-2 font-serif item-center justify-center bg-gradient-to-r from-blue-800 to-blue-600 hover:from-blue-900 hover:to-blue-700 text-white transition-all"
        size="xl"
        label="+ Add New Labs"
        @click="create_result = true"
      />
    </div>

    <div
      class="mt-3 grid grid-cols-5 gap-3 p-3"
      v-if="!create_result && result.length"
    >
      <Switch
        size="sm"
      ...
      <Badge
        :variant="'solid'"
        class="block w-full rounded-full font-serif item-center justify-center bg-gradient-to-r from-blue-800 to-blue-600 hover:from-blue-900 hover:to-blue-700 text-white transition-all"
        size="lg"
        label="+ Add New Labs"
        @click="create_result = true"
      />
          ...
          <div class="p-5 grid grid-cols-10">
            ...
            <div class="col-span-4">
              <ul
                ...
                class="grid grid-cols-3 justify-start items-center border"
              >
                <li>{{ item.test }}</li>
                <li>{{ item.result }}</li>
                <li>{{ item.uom }}</li>
    ...
    <div v-else>
      <div class="font-bold font-serif text-xl pl-2 pr-4 grid grid-cols-5 mt-5">
        ...
        <Badge
          :variant="'solid'"
          class="block w-full rounded-full font-serif item-center justify-center bg-gradient-to-r from-gray-800 to-blue-600 hover:from-blue-900 hover:to-blue-700 text-white transition-all"
          size="lg"
          label="Go Back"
      ...
      <div class="grid grid-cols-3 gap-3 font-mono">
          ...
          <div>
            <ul class="pt-3">
              <li v-for="item in lab_result_line" class="p-1">
                <span
                  class="grid md:grid-cols-5 gap-2 text-xs font-mono border p-2 hover:shadow-md bg-blue-100/30"
                >
                  ...
                  <div v-else></div>
                  <div v-if="!item.min_range" class="font-2xs text-red-700">
                    Add Min Range
                  </div>
                  <div v-else></div>
                  <div v-if="!item.max_range" class="font-2xs text-red-700">
                    Please Max Range
                  </div>
                  <div v-else></div>
                </span>
        ...
        <div class="pr-2">
          ...
          <span
            class="grid md:grid-cols-5 gap-2 text-xs font-sans border p-2 hover:shadow-md bg-green-100/30"
          >
            <FormControl
              :type="'text'"
              variant="outline"
            ...
            <FormControl
              ...
              v-model="new_lab[3]"
            />
            <div v-if="!new_lab[0]" class="font-2xs text-red-700 col-span-2">
              Add Lab Name,Dont Use "-"
            </div>
            <div class="col-span-2" v-else></div>
            <div v-if="!new_lab[1]" class="font-2xs text-red-700">
              Add Lab UOM
            </div>
            <div v-else></div>
            <div v-if="!new_lab[2]" class="font-2xs text-red-700">
              Add Min Range
            </div>
            <div v-else></div>
            <div v-if="!new_lab[3]" class="font-2xs text-red-700">
              Add Max Range
            </div>
            <div v-else></div>
  ...
  if (lab_result_date.value && lab_result_line.value.length) {
    ...
    for (let item of lab_result_line.value) {
      if (item.uom && item.result) {
        ...
        error.value =
          error.value + " " + item.label + ": Uom or result is not entered";
      }
    }
    if (!error.value) {
      make_labss.submit();
