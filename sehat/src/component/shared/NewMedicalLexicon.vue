<template>
      <div class="grid grid-cols-4 gap-3 capitalize">
        <div class="text-md font-semibold pb-1 col-span-1">{{ props.Phrase.Name }}
        </div>
  ...
  />
  <div/>
  <FormControl :type="'text'" label="Name" required="true"
      size="sm" variant="subtle" placeholder="Name" v-model="props.Phrase.Name"  class="col-span-3"/>
<!-- v-if="props.Phrase.Category=='Meds'" -->
  ...
  <div class=""/>
      ...
      <FormControl type="autocomplete" v-if="props.Phrase.Category=='labs'"
        :options="[
        {label: 'UnCategorised/No Result',value: 'No Result', description: 'Unassigned'},
    ...
    <FormControl type="textarea" v-if="props.Phrase.Category=='surg'" size="sm" variant="subtle" placeholder="General Advise about the Procedure" v-model="props.Phrase.surg_adv"/>
      ...
      <FormControl :type="'autocomplete'" :options="library.dosage_db" label="Default Dose" required="true" v-if="props.Phrase.Category=='Meds'"
      size="sm" variant="outline" placeholder="Dose" v-model="props.Phrase.med_panel[2]"/>
      <FormControl :type="'autocomplete'" :options="library.duration_db" label="Default Duration" required="true" v-if="props.Phrase.Category=='Meds'"
      ...
      <div class="text-sm capitalize py-2"  v-if="props.Phrase.Category=='Meds'">Fill in details such that it can be used universally for all future interactions, not this specific one. 
     </div><div class="text-sm capitalize pb-1" v-if="false"> Any comments or qualifiers that you may have used will be 
      picked up after you save this new term</div>
      
      <div class="flex items-end justify-end pr-6">
        <div v-if="props.Phrase.Category=='Unknown'" class="text-red-800 text-xs">Please Select a Type</div>
      <Button @click="make_newLexicon(props.Phrase)" v-if="(props.Phrase.Category!='Meds' && props.Phrase.Category!='Unknown')||(props.Phrase.Category=='Meds' && props.Phrase.med_panel[0] 
                                                              && props.Phrase.med_panel[2] && props.Phrase.med_panel[3])"
